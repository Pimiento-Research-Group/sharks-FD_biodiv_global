...
###################################################################################
carto_congruence_t2.5 <- function(nbvar=3,data="Hotspot", x="RS_tot",index2="FRiC_r",index3="PD",index4="Threats",
                               names_fig="a",xlim=c(-166,166),ylim=c(-50,55),col_pt=c("#fdae61","#EDDF19","purple"),cex_point=0.3,cex_text_fig=1,
                               cex_legend=0.5,text=c("Non hotspots values"," 2.5% of the three variables","5% of the three variables"),
                               main_legend="Congruence zone between PD,FD, and SR with values in the upper",ncol=1,
  ...
  if(nbvar==4){
    ...
    #w10 <- eval(parse(text=paste("cbind(",data3,"[which(",data3,"$",x,"==1 & ",data3,"$",index2,"==1 & ",data3,"$",index3,"==1 & ",
    #                             data3,"$",index4,"==1 ),],color=col[3])",sep="")))# hot spot
    Nothing <- eval(parse(text=paste("cbind(",data,"[which(",data,"$",x,"==0 | ",data,"$",index2,"==0 | ",data,"$",index3,"==0| ",data,"$",index4,"==0),],color='#6BAACC')",sep="")))
  }
  ...
  if(nbvar==3){
    ...
    #w5 <- eval(parse(text=paste("cbind(",data2,"[which(",data2,"$",x,"==1 & ",data2,"$",index2,"==1 & ",data2,"$",index3,"==1),],color=col[2])",sep="")))# hot spot
    #w10 <- eval(parse(text=paste("cbind(",data3,"[which(",data3,"$",x,"==1 & ",data3,"$",index2,"==1 & ",data3,"$",index3,"==1),],color=col[3])",sep="")))# hot spot
    Nothing <- eval(parse(text=paste("cbind(",data,"[which(",data,"$",x,"==0 | ",data,"$",index2,"==0 | ",data,"$",index3,"==0),],color='#6BAACC')",sep="")))# hot spot
  }
  ...
  if(nbvar==2){
   ...
   #w5 <- eval(parse(text=paste("cbind(",data2,"[which(",data2,"$",x,"==1 & ",data2,"$",index2,"==1),],color=col[2])",sep="")))# hot spot
  #w10 <- eval(parse(text=paste("cbind(",data3,"[which(",data3,"$",x,"==1 & ",data3,"$",index2,"==1),],color=col[3])",sep="")))# hot spot
    Nothing <- eval(parse(text=paste("cbind(",data,"[which(",data3,"$",x,"==0 | ",data,"$",index2,"==0),],color='#6BAACC')",sep="")))# hot spot
  }
...
carto_function <- function(y=Hotspot,x="RS_tot",index2="FRiC_new",names_fig="a",pos_legY=87,pos_legX=0.5,
                           ...
                           xlim=c(-166,166),pos_leg="topright",ylim=c(-83.5,83.5),ncol=2,...){
    
  w <- eval(parse(text=paste("cbind(y[which(y$",x,"==1 & y$",index2,"==1),],color='#B70F0D')",sep="")))# hot spot
  yy <-eval(parse(text=paste("cbind(y[which(y$",x,"==0 & y$",index2,"==1),],color='#fdae61')",sep="")))# hot spot FNPE
  z <- eval(parse(text=paste("cbind(y[which(y$",x,"==1 & y$",index2,"==0),],color='#EDDF19')",sep=""))) # hot spot richness
  ...
  colour=c("#B6E0EE","#6BAACC","#fdae61","#EDDF19","#B70F0D")
  legend(pos_leg,leg=c("No values",text),col="black",pt.bg=colour,pch=22,cex=cex_legend,lty=NA,
         lwd=0.2,box.lwd=0.6,pt.cex=pt.cexleg,ncol=ncol,bg="#FFFFFF95",x.intersp=0.1,...)
...
Map_the_world_mpa <- function (Data_PA=grid$P_A, coord_X=grid$X_ENTIER,coord_Y=grid$Y_ENTIER,
                           col=c("White","yellow","red3"),breaks=seq(0,40,2),include.lowest=T,
                           xlim=c(-166,166),ylim=c(-83.5,83.5),zlevels=6,main_legend_cex=1.2,
                           coastline=T,main_legend="Species Richness",output=F,legend_increment=100,
                           names_fig="a)",cex_names_fig=1,...){
...
#############################################
#									                          #
#      World mapping fonction  bicolor      #
#                                           #
#                                           #
#############################################

Map_the_world_RGB<- function (coord_X = grid$X_ENTIER,coord_Y = grid$Y_ENTIER, col = color, xlim = c(-166,166), ylim = c(-83.5,83.5),coastline = TRUE, output=FALSE, names_fig="1MYA",cex_names_fig=1, cex_points=1.22, names_fig_2="A", cex_names_fig_2=0.5, names_fig2_pos=100,...){
  require(shape)
  ...
  if (output==TRUE) {
    ...
    points(coord_X,coord_Y,col = col, pch = 15, cex=cex_points) 
    
    if (coastline == TRUE) {
      plot(coast,col="black",add=TRUE)
    } # end of if
    
    axis(side=1,line=0,cex.axis=1,lwd=0.35,tcl=-0.25,bg="white",labels=c("-180","-135","-90","-45","0","45","90","135","180"),at=c(-179,-135,-90,-45,0,45,90,135,179),mgp=c(3, 0.5, 0))
    axis(side=2,line=0,las=2,cex.axis=1,lwd=0.35,tcl=-0.25,bg="white",labels=c("-90","-45","0","45","90"),at=c(-90,-45,0,45,90),mgp=c(3, 0.5, 0))
    
    mtext(names_fig,side=3,line=-1.5,at= 165,cex=cex_names_fig,bg="white",las=1, font=2) # 1MYA  	
    
    mtext(names_fig_2,side=3,line=0,at=names_fig2_pos,cex=cex_names_fig_2,bg="white",las=1, font=2)  # A, B, C 
